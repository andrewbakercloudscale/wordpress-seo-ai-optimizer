...
const PANELS = [
    ...
    { tab: 'catfix',  cardClass: 'ab-card-catmig',          file: 'panel-catmig.png',         label: '🔄 Migrate Categories',           expand: false, cmLoad: true, trimRows: true },
    // Batch Tab
...
<ul>
<li><strong>Site name</strong>: Your website's name as it should appear in Google Search results and social media link previews. This is the brand name Google associates with your content, so make it consistent with how you refer to your site everywhere else.</li>
...
<li><strong>Home title</strong>: The specific SEO title for your homepage. Your homepage is usually your most important page for brand searches, so write a title that includes your name or brand and your main topic area, e.g. <em>"Andrew Baker: Cloud Architecture &amp; Technology Leadership"</em>.</li>
...
<p>The meta description is the short snippet that appears under your page title in Google Search results, usually two or three lines describing what the page is about. It is often the deciding factor in whether a searcher clicks your result or the one below it. When you don't write one, Google generates its own, usually by pulling a random sentence from your page content. That sentence is rarely your best pitch to a potential reader.</p>
...
<li><strong>Regenerate All</strong>: Forces new descriptions for every post, replacing whatever is already there. Use this if you've changed your custom prompt or want a fresh pass across your entire site. This will overwrite manually written descriptions, so use with care.</li>
...
<p>ALT text is also generated automatically during meta description generation when you use Generate Missing or Generate in the Meta Description Writer. If you've already run that, your images may already have ALT text. This dedicated panel is for auditing and bulk-processing images specifically.</p>
<ul>
...
<li><strong>Match by Categories</strong>: Posts in the same category count as more relevant. Recommended. Categories are your primary topical grouping signal.</li>
...
'ab-card-rc-table': `
<p>The <strong>Related Articles Management</strong> table is your control panel for the related articles network. When you first enable the feature, all posts start as "Pending". They haven't been processed yet and won't show related article blocks on the frontend. Run the batch operation here to process your entire site and go from zero to a complete internal linking network in one click.</p>
...
<li><strong>Generate &amp; Sync</strong>: Processes all Pending posts and updates link counts for Complete posts. This is the main batch button. Run it after enabling the feature for the first time, or after adding new posts that haven't been processed yet.</li>
...
<p>A sitemap is the most direct line of communication between your website and Google. When you publish a new post, Google might not discover it for days or weeks if it has to find the page by following links. With a sitemap, you're effectively handing Google a complete, up-to-date list of every important URL on your site. Google re-crawls that list regularly to find new content fast.</p>
...
<p><strong>Critical distinction:</strong> robots.txt controls <em>crawl access</em>, not <em>indexing</em>. A page you block in robots.txt may still appear in Google's search results if other sites link to it. Google knows it exists but cannot read it. If you want a page removed from Google's index, use the noindex meta tag (configured in the SEO Features panel) instead of robots.txt. If you want both (no crawling and no indexing), you need both.</p>`,
...
'ab-card-https': `
<p>When a site moves from HTTP to HTTPS, the server-side redirect ensures visitors always land on the secure version. But the database is full of old hardcoded <code>http://</code> links: image URLs embedded in post content, internal links written years ago, and theme option values. These old HTTP references cause "mixed content" browser warnings, where the browser detects that a secure page is loading resources over an insecure connection and blocks or flags them. Mixed content warnings can also suppress the padlock icon in the browser's address bar, eroding visitor trust.</p>
...
'ab-card-render': `
<p>Page load speed is a confirmed Google ranking factor, and Google measures it using Core Web Vitals, the real-world performance metrics visible in Google Search Console. The three optimisations in this panel target specific speed metrics that appear in those reports. None of them change how your site looks. They only change how quickly it gets to a usable state in the browser.</p>
<ul>
...
<p>The <strong>Category Fixer</strong> audits every published post on your site and compares its content to its current category assignment. It uses keyword scoring to identify mismatches and can also use your configured AI provider for more nuanced analysis of ambiguous cases. The result is a prioritised list of suggested recategorisations. Review each one and decide whether to apply it.</p>
<p><strong>How to use it:</strong></p>
<ol>
...
<p><strong>Filter tabs</strong> at the top let you narrow the view to: All posts, Changed (suggestions pending), Unchanged (post is already well-categorised), Low-confidence (suggestions the algorithm is less certain about), and Uncategorised (posts with no category at all, which should be addressed first).</p>
...
'ab-card-cathealth': `
<p>Not all categories are created equal. A category with 25 posts about AWS architecture sends a strong topical signal to Google. A category with one post from 2021 that was never followed up sends essentially no signal at all, and it wastes crawl budget. Google allocates a limited amount of crawl budget to each site, and spending some of it on thin, empty category pages is a wasted opportunity to get your actual content crawled more frequently.</p>
<p>The <strong>Category Health Dashboard</strong> gives you an at-a-glance quality grade for every category on your site, so you can see immediately which categories are working for you and which ones are dragging down your topical authority. The grading is based on post count and recency. A category you stopped writing about three years ago signals to Google that your site has abandoned that topic.</p>
...
'ab-card-catmig': `
<p>Over time, most WordPress sites accumulate categories that have grown thin: a category you tried once with three posts, a duplicate of something already covered elsewhere, or a topic that never had enough content to build real topical authority. These thin categories dilute your site's authority signal. Google sees a category with two or three posts and concludes you're not a serious resource on that topic.</p>
<p>The <strong>Migrate Categories</strong> panel lets you retire weak categories cleanly. Instead of manually reassigning each post and then hunting down the empty category to delete it, this panel does everything in one place: shows you all the candidates for retirement (sorted by fewest posts first), lets you choose what to do with each post, and then deletes the empty category when you're done.</p>
<p><strong>How to use it:</strong></p>
<ol>
<li>Click <strong>🔄 Load Categories</strong>. Categories are listed from fewest posts to most — the lowest-count ones are the best candidates to migrate away.</li>
<li>Click a category row to open the post list for that category.</li>
<li>For each post, set the action using the dropdown:
  <ul>
    <li><strong>Remove</strong>: Removes this category from the post, leaving its other categories intact. Only available for posts already assigned to two or more categories.</li>
    <li><strong>Swap → [another category]</strong>: Replaces this category with a different one. This is the required action for posts assigned to only this category — removing would leave them uncategorised.</li>
  </ul>
</li>
<li>Click <strong>Apply</strong> on individual rows, or <strong>✓ Apply All</strong> to process every pending row at once.</li>
<li>Once all posts have been migrated away, a red <strong>🗑 Delete Category</strong> button appears in the header. Click it to permanently delete the now-empty category — no need to leave the plugin.</li>
</ol>
<p>Click <strong>← Back to Categories</strong> to return to the category list and start working on a different category. The category list refreshes automatically so already-emptied categories disappear from the list.</p>
<p><strong>When to use this panel:</strong> Run the <strong>Category Health</strong> panel first to identify your weakest categories (grade D or F with very few posts). Those are the prime candidates to migrate. Cross-reference with <strong>Category Drift Detection</strong> to see whether a low-count category is better merged into an adjacent topic rather than deleted outright. Migrate Categories is the execution step — use it after you've decided which categories to retire.</p>`,
...
(async () => {
    ...
    for (const panel of PANELS) {
        ...
        // Migrate Categories: click Load Categories and wait for the category table to appear
        if (panel.cmLoad) {
            const loadBtn = page.locator('#cm-load-btn');
            if (await loadBtn.isVisible().catch(() => false)) {
                await loadBtn.click();
                await page.waitForFunction(
                    () => { const w = document.getElementById('cm-cat-wrap'); return w && w.querySelector('table'); },
                    { timeout: 20000 }
                ).catch(() => {});
                await page.waitForTimeout(800);
            }
    ...
    const { id, url } = await createOrUpdatePage('CloudScale SEO AI Optimizer', finalContent, 'seo-ai-optimizer', parentId);
