...
echo "  sudo aws s3 cp s3://andrewninjawordpress/cloudscale-seo-ai-optimizer.zip /tmp/plugin.zip && sudo rm -rf /var/www/html/wp-content/plugins/cloudscale-seo-ai-optimizer && sudo unzip -q /tmp/plugin.zip -d /var/www/html/wp-content/plugins/ && sudo chown -R apache:apache /var/www/html/wp-content/plugins/cloudscale-seo-ai-optimizer && php -r \"if(function_exists('opcache_reset'))opcache_reset();\""
